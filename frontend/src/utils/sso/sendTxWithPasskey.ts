...
import { loadExistingPasskey } from "./passkeys";

const GAME_CONTRACT_ADDRESS = import.meta.env
  .VITE_GAME_CONTRACT_ADDRESS as `0x${string}`;

export const sendWithPasskey = async (
  data: `0x${string}`,
  rpcClient: PublicClient,
  authorizeTransaction: AuthorizeTxFn,
  value?: bigint,
) => {
...
async function sendTxWithPasskey(
  ...
  console.log("passkeyCredentials", passkeyCredentials)
  console.log("txData", txData)
  console.log("authorizeTransaction", authorizeTransaction)
  if (!readClient) {
  ...
  const authenticatorDataHex = toHex(
    base64UrlToUint8Array(response.authenticatorData),
  );
  ...
  const signatureData = unwrapEC2Signature(
    base64UrlToUint8Array(response.signature),
  );
  ...
  packedUserOp.signature = concat([
    ssoContracts.webauthnValidator,
    passkeySignatureEncoded,
  ]);
  ...
  const rpcRequest = readClient.request as unknown as (
    args: RpcRequestArgs,
  ) => Promise<unknown>;
